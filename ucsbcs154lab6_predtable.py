...
el_state = pyrtl.WireVector(bitwidth=2, name="el_state") 
new_pred_state = pyrtl.WireVector(bitwidth=2, name="new_pred_state")

el_state <<= pred_state[update_branch_pc[2:5]]

with pyrtl.conditional_assignment:
    with pred_state[fetch_pc[2:5]] >= 2:
        pred_taken |= 1
    with pyrtl.otherwise:
        pred_taken |= 0

pred_state[update_branch_pc[2:5]] <<= new_pred_state

with pyrtl.conditional_assignment:
    with update_prediction:
        with update_branch_taken:
            with el_state == 3:
                new_pred_state |= el_state
            with pyrtl.otherwise:
                new_pred_state |= el_state + 1
        with pyrtl.otherwise:
            with el_state == 0:
                new_pred_state |= el_state
            with pyrtl.otherwise:
